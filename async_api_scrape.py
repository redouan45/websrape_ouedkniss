    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        ...
        for page in cars:
            for car in page:
                ...
                phone_tasks.append(session.post(  url, headers=headers_phone, data=payload_phone))
        segment_car_task = []
        segment_phone_task = []
        for i in range(0,len(car_tasks)):
            if  i%48 ==0 :
                ...
                result_phone = await asyncio.gather(*segment_phone_task)
                for item in result_items:
                    car_data.append(await item.text())
                for phone in result_phone:
...
cars_json , phone_nums = asyncio.run(get_data(cars))
with open('Data.txt' ,'a+',encoding='utf8') as file :
    for j in range(0, len(cars_json)):
        file.write(str(cars_json[j]))
        file.write(str(phone_nums[j]))
